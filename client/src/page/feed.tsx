...
// Lazy initialization of Mermaid
...
export function FeedPage({id, TOC, clean}: { id: string, TOC: () => JSX.Element, clean: (id: string) => void }) {
    ...
    const deleteFeed = useCallback(() => {
        showConfirm(
            ...
            async () => {
                if (!feed) return;
                try {
                    const {error} = await client.feed({id: feed.id}).delete(null, {
                    ...
                    showAlert(t("delete.failed"));
                }
            }
        );
    }, [feed, showConfirm, showAlert, setLocation, t]);

    const topFeed = useCallback(() => {
        const isUnTop = !(top > 0);
        ...
        showConfirm(
            ...
            async () => {
                if (!feed) return;
                try {
                    const {error} = await client.feed.top({id: feed.id}).post(
                        {top: topNew},
                        {headers: headersWithAuth()}
                    );
                    ...
                    console.error("Error setting top status:", e);
                    showAlert(t("article.top.failed"));
                }
            }
        );
    }, [feed, top, showConfirm, showAlert, t]);
        ...
        client
            ...
            .then(({data, error}) => {
                if (error) {
                    ...
                    // Extract head image - do this only once when data is loaded
                    const img_reg = /!\[.*?\]\((.*?)\)/;
    ...
    // Process Mermaid charts when content is available
    useEffect(() => {
        if (!feed) return;
        ...
        const timer = setTimeout(() => {
            ...
            try {
                // Process default theme charts
                mermaid.initialize({startOnLoad: false, theme: "default"});
                mermaid.run({
                    suppressErrors: true,
                    nodes: contentRef.current.querySelectorAll("pre.mermaid_default")
                })
                    .then(() => {
                        // Then process dark theme charts
                ...
                console.error("Error in Mermaid processing:", e);
            }
        }, 100);
                ...
                {feed && !error && (
                    <>
                        ...
                        {feed.uv}
                                ...
                                {/* Content with ref for mermaid processing */}
                                <div ref={contentRef}>
            ...
            {config.get<boolean>('comment.enabled') &&
                <div className="m-2 flex flex-col justify-center items-center">
                    ...
                    {error && (
                        <>
                            <div
                                className="flex flex-col wauto rounded-2xl bg-w t-primary m-2 p-6 items-center justify-center">
                                <h1 className="text-xl font-bold t-primary">{error}</h1>
                                <button
                                    className="mt-2 bg-theme text-white px-4 py-2 rounded-full"
